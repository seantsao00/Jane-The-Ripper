 ...
 * Error names.
 */
static const char *const rules_errors[] = {NULL, /* No error */
...
static struct {
     ...
     */
    union {
        char buffer[3][RULE_WORD_SIZE * 2];
        long dummy;
    } aligned;
    /*
...
#define POSITION(pos)                                                                          \
    {                                                                                          \
        if (((pos) = rules_vars[ARCH_INDEX(RULE)]) == INVALID_LENGTH) goto out_ERROR_POSITION; \
    }
...
#define CLASS_export_pos(start, true, false)                                    \
    {                                                                           \
        char value, *cls;                                                       \
        if ((value = RULE) == '?') {                                            \
            if (!(cls = rules_classes[ARCH_INDEX(RULE)])) goto out_ERROR_CLASS; \
            for (pos = (start); ARCH_INDEX(in[pos]); pos++)                     \
                if (cls[ARCH_INDEX(in[pos])]) {                                 \
                    true;                                                       \
                } else {                                                        \
                    false;                                                      \
                }                                                               \
...
static void rules_init_class(char name, char *valid) {
    ...
    rules_classes[ARCH_INDEX(name)] = (char *)malloc(0x100);
    ...
    if ((name | 0x20) >= 'a' && (name | 0x20) <= 'z') {
        inv = name ^ 0x20;
        rules_classes[ARCH_INDEX(inv)] = (char *)malloc(0x100);
...
static char *rules_init_conv(char *src, char *dst) {
    ...
    conv = (char *)malloc(0x100);
    for (pos = 0; pos < 0x100; pos++) conv[pos] = pos;

    while (*src) conv[ARCH_INDEX(*src++)] = *dst++;
...
void rules_init(int max_length) {
    ...
    if (max_length > RULE_WORD_SIZE - 1) max_length = RULE_WORD_SIZE - 1;
...
__device__ char *rules_apply(char *word, char *rule, int split, char *last) {
    char *in, *alt, *memory = word;
    int length;
    int which;

    in = buffer[0];
    if (in == last) in = buffer[2];
    ...
    while (RULE) {
        if (length >= RULE_WORD_SIZE) in[length = RULE_WORD_SIZE - 1] = 0;
        ...
        switch (LAST) {
            ...
            case 'c': {
                ...
                if ((in[0] = conv_toupper[ARCH_INDEX(in[0])]))
                    while (in[++pos]) in[pos] = conv_tolower[ARCH_INDEX(in[pos])];
                in[pos] = 0;
            } break;
            ...
            case 'p':
                if (length < 2) break;
                {
                    ...
                    if (strchr("sxz", in[pos])
                        ...
                        strcat(in, "es");
                    else if (in[pos] == 'f' && in[pos - 1] != 'f')
                        strcpy(&in[pos], "ves");
                    else if (pos > 1 && in[pos] == 'e' && in[pos - 1] == 'f')
                        strcpy(&in[pos - 1], "ves");
            ...
            case '/': {
                int pos;
                CLASS_export_pos(0, break, {}) rules_vars['p'] = pos;
                if (in[pos]) break;
            }
                REJECT
                break;
            ...
            case 'C': {
                ...
                if ((in[0] = conv_tolower[ARCH_INDEX(in[0])]))
                    while (in[++pos]) in[pos] = conv_toupper[ARCH_INDEX(in[pos])];
                in[pos] = 0;
            } break;
            ...
            case '%': {
                ...
                POSITION(required)
                CLASS_export_pos(0, if (++count >= required) break, {}) if (count < required)
                    REJECT rules_vars['p'] = pos;
            } break;
            ...
            case 'D': {
                ...
                POSITION(pos)
                if (pos < length) {
                    memmove(&in[pos], &in[pos + 1], length - pos);
                    length--;
                }
            } break;
            ...
            case '{':
                if (length) {
                    ...
                    while (count >= length) count -= length;
                    if (!count) break;
                    GET_OUT
            ...
            case '1':
                if (split < 0) goto out_ERROR_UNALLOWED;
                if (!split) REJECT
                if (which)
                    memcpy(buffer[2], in, length + 1);
                else
                    strnzcpy(buffer[2], &word[split], RULE_WORD_SIZE);
                length = split;
                if (length > RULE_WORD_SIZE - 1) length = RULE_WORD_SIZE - 1;
...
out_OK:
    ...
    if (last) {
        if (length > rules_max_length) length = rules_max_length;
        if (length >= ARCH_SIZE - 1) {
            if (*(long *)in != *(long *)last) return in;
            if (strcmp(&in[ARCH_SIZE - 1], &last[ARCH_SIZE - 1])) return in;
            return NULL;
        }
